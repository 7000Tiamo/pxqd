...
COMMENT ON COLUMN training_enrollments.training_id IS '培训ID，关联trainings表';
COMMENT ON COLUMN training_enrollments.user_id IS '用户ID，关联users表';
...
COMMENT ON COLUMN training_checkins.training_id IS '培训ID，关联trainings表';
COMMENT ON COLUMN training_checkins.user_id IS '用户ID，关联users表';
...
INSERT INTO users (username, name, role, status, password, created_at, updated_at)
SELECT 'admin', '管理员', 'admin', 1, 'e10adc3949ba59abbe56e057f20f883e', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username='admin');

-- 默认普通用户：用户名 employee，密码 123456（MD5 简单示例：e10adc3949ba59abbe56e057f20f883e）
INSERT INTO users (username, name, employee_no, dept, phone, role, status, password, created_at, updated_at)
SELECT 'emp001', '张三', 'E001', '技术部', '13800138000', 'employee', 1, 'e10adc3949ba59abbe56e057f20f883e', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP
